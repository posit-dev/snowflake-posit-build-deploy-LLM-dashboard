...
Now that we've done some exploratory data analysis, let's create our interactive dashboard using Snowflake Cortex AI. We'll first test that ellmer can connect to Cortex AI, then build a custom Shiny app that you can deploy to Posit Connect.
...
your_snowflake_account <- "YOUR_ACCOUNT"  # Replace with your Snowflake account for Connect deployment
...
The {querychat} package provides a ready-to-use chat interface for data exploration. If you want to interactively explore the data before building your custom app, you can run querychat in a separate R session:

```r
# Run this in a separate R session or console
library(querychat)
...
**Note:** `querychat_app()` launches an interactive application that will block further code execution. Close the app when you're done exploring to continue with the next steps.
...
get_connection <- function() {
  ...
  # Running in Posit Connect (deployed app)
    ...
    con <- DBI::dbConnect(
      odbc::snowflake(),
      account = "%s",  # Your Snowflake account from settings
      warehouse = warehouse,
      database = database,
      schema = schema,
      authenticator = "OAUTH",
...
', your_snowflake_account, cortex_model)
...
message("- Cortex AI Model: ", cortex_model)
message("- Snowflake Account: ", your_snowflake_account)
